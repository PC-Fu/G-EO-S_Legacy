...
namespace GeometryUtilities
{
   ...
   * @param[in] pointIndices list of index references for the points array in order (CW or CCW) about the polygon loop
                           ...
                           R1Tensor& normal )
  {
    ...
    if( n>2 )
    {
      ...
      else
      {
        ...
        std::cout << "Randy's bug: area = " << area << std::endl;
        for( lArray1d::size_type a=0 ; a<n ; ++a ) std::cout << points[pointsIndices[a]](0) << " "
    ...
    else if( n==2 )
    {
      ...
      //For 2D elements, a face is actually an edge with two nodes. We treat the length of this edge as the surface area and use it in the calculation of tractions.
      R1Tensor x1_x0;
   ...
   * @param[in] pointsEntries list of index references for the points array in order (CW or CCW) about the polygon loop
...
//    {
  //    std::cout.precision(15);
  //    std::cout<<"  FaceID "<<faceIndex<<": tolerances reached on iteration "<<count<<std::endl;
  //    std::cout<<"  Force = "<<force<<std::endl;
  //    std::cout<<"  xap = "<<xap<<std::endl;

 //     std::cout<<count<<"    "<<S<<" | "<<dS<<" | "<<G<<" | "<<Dot(G,G)<<" | "<<Gtol<<std::endl;
//    }
                                           ...
                                           R1Tensor& pointproj)
  {
    ...
    //scale it by (the normal distance of the point / normal component of the vproj)
   ...
   *
   * Parallel segments as well as degenerate segments (i.e., collocated termini) are handled in the logic
   * The point of intersection for the skew case is determined using dot products of the normal component
   *
   ...
   * @param[out] intersection The u-v coordinates of the intersection (if it exists)
   * @param[out] intersection2 The u-v coordinates of the second intersection (if it exists ... only for parallel segment overlap)
   * @return Contact state: 0 if skew intersection, 1 if point to line intersection, 2 if parallel segment overlap, 3 if not intersected
   */
                               ...
                               const realT small)
  {
    ...
    realT dnuv21 = 0., dnuv2p1 = 0., dd = 0.0;
    {
      ...
      //if the normal projection of segment 2 has no length, the segments are parallel
      //now, we need to check cases and see whether the segments overlap
      if( isEqual(dd, 0) )
      {
        //if the segments are parallel but not on the same line, there is no contact
        ...
        if(pb<0) return 3; // no contact
        if(pa>1) return 3; // no contact
    ...
    //project intersection onto line segment 1
    {
      ...
      //dd now holds the squared distance between node 1's first node and the intersection
      //divided by the squared length of segment 1, so now check to make sure the
      //(normalized) distance (squared) is within tolerance ... i.e., the intersection point
  ...
  /**
   * @brief Determine whether the 2D point is in the 2D face defined by ordered points
   * @author Scott Johnson
   *
   * The point in face is the cumulative OR for point in triangles composing the face
   *
                         ...
                         const bool planarFaces)
  {
    ...
    if(planarFaces)
    {
      for (localIndex i = 0; i < toFaces.Dimension(1); i++)
      {
        ...
        //make sure the point does not lie outside of the planar approximation of the face
        R1Tensor d(point);
        d -= center;
        if (Dot(normal, d) > 0)
          return false;
    ...
    else
    {
      for (localIndex i = 0; i < toFaces.Dimension(1); i++)
      {
        ...
        //go through each edge, and use the face center and element center to construct tets
      ...
      //if the point does not lie in any of the constituent tets, then it is outside
      return false;
    }
  }
  ...
  /**
   * @author Scott Johnson
   * @brief Finds the polygon of intersection for two convex polygons of the same handedness for the special case where the polygons are coincident
  ...
  /**
   * @author Scott Johnson
   * @brief Determines the intersection points between two convex polygons of the same handedness
   *
   * Intersections are determined using an O(N^2) technique comparing each segment to each segment
   * Parallel segments as well as degenerate segments (i.e., collocated termini) are handled in the logic
   * of LineIntersection
   *
    ...
    for (Array1dT<R1TensorT<2> >::size_type i1 = 1; i1 <= a1.size(); ++i1)
    {
      ...
      for (Array1dT<R1TensorT<2> >::size_type i2 = 1; i2 <= a2.size(); ++i2)
      {
        ...
        localIndex contactType = LineIntersection(a1[i1-1],a1[ii1],a2[i2-1],a2[ii2],
                                                  intersection, intersection2, 0.0);
  ...
  /**
   * @brief Order the polygon points in counter-clockwise order, and remove all redundant points
   * @author Scott Johnson
   *
   * Uses a generalized Graham Scan.
   * The given polygon points are ordered in CCW direction by obtaining the dot product
   * with the pivot vector (in the non-pathological case, this is the unit vector along 0-1,
   * but the logic admits the non-ideal case, too); for descending dot product, the angular
   * displacement from the pivot increases. Because this is a convex loop, the logic should hold.
   *
   * Ordering is secondarily dependent on the distance from the pivot node, such that, for any set
   * of nodes with the same angular displacement, that with the largest distance is kept, while the
   * others are removed. This produces a minimal description of the convex polygon.
   *
  ...
  void OrderPoints_2DPolygon(Array1dT<R1TensorT<2> >& polygonPoints, const realT tol  )
  {
    ...
    //(3) if the number of polygon points has been reduced below the threshold, return
    if(arr.size() < 4)
      return;

    //(4) get angle values about the pivot
    {
      //get the additional vectors and find the cross products and dot products (sine and cosine)
      R1TensorT<2> value;
      while(iter != arr.end())
      {
        value = (*iter).m_point;
        ...
        //the "angle" metric is in the range [1,-3) in counter-clockwise order (note: at cosine=+/-1 : sine can be +/-0)
        (*iter).m_angle = sineValue >= 0 ? cosineValue : -2 - cosineValue;//1,0,-1,-2,(-3 or 1)
    ...
    //(4) sort points: counter-clockwise will go most positive to most negative, starting with pivot as the first point
    //    sorting is primary on dot product and secondary on distance
    std::sort(arr.begin()+1,arr.end(), PointStruct::Compare);

    //(5) polygonPoints is now ordered, so go through and remove those with duplicate angle values; note that the first
    //    (i.e., that with the largest distance value of the set of equal dot product values) will be kept while the others
   ...
   * --unlimited release--
   * Didonato (1993) "An algorithm to find the intersection of two convex polygons"
   * AD-A274 722 NSWC (Dahlgren Div., Dahlgren, VA 22448)
   * --unlimited release--
   * The same general workflow is used, but the method of determining intersections and inclusions is different
   * as is the method of ordering the resultant point loop to define the convex polygon of overlap
   * This is probably publishable as its own paper, and I will update the citation when I have a chance to write it up
   *
   * @param[in] a1 Local planar coordinates of the projected face 1
   * @param[in] a2 Local planar coordinates of the projected face 2
                                ...
                                Array1dT<R1TensorT<2> >& polygonPoints)
  {
    ...
    //(1) first, if either polygon is 1D or less, there is, by definition, no contact area
    if(a1.size() < 3 || a2.size() < 3)
      return 0.0;

    //(2) second, if the bounding boxes in local frame don't overlap, there is no contact area
    {
    ...
    //(3) Are all of the points of 1 contained in 2? -> YES, then the overlap is 1 RETURN
    ...
    //(4) Are all of the points of 2 contained in 1? -> YES, then the overlap is 2 RETURN
    if(!ret)
    {
   ...
   * @author Scott Johnson
   * @param[in] pointsEntries list of indices in the point references and displacements list
   * @param[in] pointReferences list of reference positions
   * @param[in] pointDisplacements list of displacements of the point from its reference position
   * @param[in] e1 unit vector in the plane
   * @param[in] e2 unit vector in the plane orthogonal to e1
   * @param[out] localPoints coordinates of the given points in the e1 and e2 coordinates
   ...
   * @param[in] normal plane normal
   * @param[in] pointOnPlane a point that lies on the plane and is inside the projection of the polygon on the plane
   * @param[in] points ordered list of points that describes the boundary of the polygon
   ...
   * @param[out] inContact list of triangles for each face overlapping the plane (0 points ... or 2 points with the pointOnPlane implied as the terminus)
   * @param[out] outContact list of triangles for each face not overlapping the plane (0 points ... or 2 points with the pointOnPlane implied as the terminus)
  ...
  /**
   * @brief Clip a 3D convex polygon by a plane: only provides an approximation of the in and out of contact areas
   * @author Scott Johnson
   * Provides both the inside and outside surfaces as a tesselation
   * @param[in] normal plane normal
   * @param[in] pointOnPlane a point that lies on the plane and is inside the projection of the polygon on the plane
   ...
   * @param[out] inContact list of points along the edge that are in contact or intersection with the common plane
   * @param[out] outContact list of points along the edge that are out of contact or are intersecting with the common plane
   * @param[out] inDistance list of signed distances to the common plane for each of the in contact points
   * @param[out] outDistance list of signed distances to the common plane for each of the out of contact points
   * @param[in] reverse Option to specify that both the polygon loop order should be reversed and the normal inverted
   */
  void ClipByPlane_3DPolygon(const R1Tensor& normal,
                             const R1Tensor& pointOnPlane,
  ...
  ///Get the strike scalar for the given unit strike vector (both in the plane of the ground)
  /**
