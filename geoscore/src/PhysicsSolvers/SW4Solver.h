...
class SW4Solver : public SolverBase
{
 public:
  ...
  void SetMaxStableTimeStep( PhysicalDomainT& domain,
			     const sArray1d& namesOfSolverRegions,
			     SpatialPartition& partition  );
...
// k-index such that the grid can be considered Cartesian for k >= m_kcart
 ...
 //Grid spacing, same in all directions.
  ...
  // Grid is given by x_i= (i-1)*dx + m_xmin_global, i=-nghost+1,..,N+nghost, where
  // the domain is i=1,..,N, and x_1=m_xmin_global, x_N = m_xmax_global. Similarly for y_j and z_k.
  realT m_xmin_global, m_xmax_global, m_ymin_global, m_ymax_global, m_zmin_global, m_zmax_global;

  // Boundary conditions 0-no boundary (parallel overlap), 1-Far field supergrid boundary, 2-free surface.
  int m_bctype[6];
